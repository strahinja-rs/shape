---
name: loop
description: Loop shape framer for scheduled-recurrence work — same prompt fires on a cadence, state accumulates between fires. ALWAYS invoke this skill when the user asks to schedule, recur, repeat, poll, monitor, watch, cron, periodically check, daily/weekly run, every-N-min run, or describe work that should fire on a temporal cadence rather than once. Distinct from shape:contract (Contract's loop is internal verify-execute within one task; Loop's loop is temporal across sessions). Produces a Loop contract — trigger cadence (fixed interval, cron, self-paced), per-fire prompt or skill invocation, accumulated state across fires, termination, dead-loop detection. Recommends /loop for in-session self-paced and fixed-interval recurrence; recommends /schedule for remote-cron scheduled agents. Does not execute or wire. Skip for one-time work (use shape:one-shot), event-triggered work (use shape:event for when-X-happens), or in-task verify-execute iteration (use shape:contract).
# description-style: directive + negative constraint (Seleznov)
# rationale: Loop framing competes with default Claude behavior (would otherwise hand-wave "set up a recurring check"); directive style forces explicit cadence + state + termination decisions. Many user workflows are scheduled-recurrence-shaped (daily summary, weekly review, hourly status check) — making the shape explicit prevents drift between intent and what gets wired.
---

<!--
ACTIVATION TESTS (for /skill:validate --bench):
1. SHOULD activate: "Every Monday at 9am, summarize last week's commits"
2. SHOULD activate (knowledge work): "Daily, check my reading list and surface anything new I should read first"
3. SHOULD NOT activate: "When a new SEF invoice arrives, list it" (event-triggered, not scheduled — shape:event)
...
- Examples (coding): nightly CI status check, hourly deploy poll, daily test-coverage report, weekly dependency-update scan.
- Examples (knowledge work): daily inbox summary, weekly research-feed digest, hourly Slack/email triage, monthly portfolio review, periodic wiki-note revisit ("what did I learn this week").
- Examples (life ops): daily voltage check, weekly orient session, monthly tax review.
- Composing with other shapes: each tick is itself a Pipeline (check → analyze → notify), or a Critic (review accumulated state and decide if action needed), or fires into a Gated handler (auto-detect candidates, gate the action).

## When NOT to Use

- One-time work — use `shape:one-shot`.
- Triggered by an event rather than a clock — use `shape:event` ("when X happens, do Y" is Event, not Loop).
- Within-task verify-execute iteration — use `shape:contract` (Loop Contract).
- In-session retry loops where the agent decides next step based on prior result — that's `shape:pipeline` with branching, not Loop.
...
- Internal composition: <none | each fire is shape:pipeline | …>
...
- **Wiring inside the skill.** The skill is frame-only. Wiring delegates to `/loop` or `/schedule`.

## Rules

- The skill always outputs a single Markdown contract file; it never wires or runs the loop.
- Every Loop contract must declare: cadence, per-fire action, accumulated state, termination, dead-loop detection, wiring path.
